...
public class MedicoController {
    ...
    public ResponseEntity<MensajeDTO> atenderCita(@PathVariable int codigo) throws Exception {
        ItemCitaMedicoDTO citas=medicoServicio.atenderCita(codigo);
        System.out.println(citas);
        return ResponseEntity.ok().body( new MensajeDTO(false, citas) );
    }

    @PostMapping("/finalizar-cita/")
    public ResponseEntity<MensajeDTO> finalizarCita(@RequestBody FinalizarCita finalizarCita) throws Exception {
        MensajeDTO mensaje=medicoServicio.finalizarCita(finalizarCita);
